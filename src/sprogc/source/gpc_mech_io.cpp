                                  ...
                                  const int verbose)
{
	...
	ReadChemkin(filename,mech,thermofile, verbose);
 ...
 *  comments and converts lines endings to "\n".
 *  Floating point number of the form 0.00D+-00 are converted to 0.00E+-00
 *  for convenience when working with data generated by Fortran progrms.
 */
 ...
 * The first stage is to read the entire file contents into a string and
 * do some basic cleaning \ref loadCK_file .
 *
 */
 ...
 * a string.
 *
 *@param[in]        thermo    data read from file by \ref extractCK_Thermo
 *@param[in,out]   mech      mechanism object into which data should be read
 *
 * In the following 'line' refers to the characters coming between two `\n` characters.
 *
 * Notes on contents and format of data:
 *
 * The first line (this does not include the THERMO keyword) should consists of three 10
 * character columns.  The columns should contain three temperatures in increasing order,
 * these define two ranges over which different thermodynamic coefficients will be used.
 * The remainder of the data should be in four line blocks.  The first line of each block
 * should begin with the species name, which is space terminate; blocks relating to species
 * not in the mechanism are skipped.  Starting in what is typically column 29 (but possibly
 * this seems to be based on an offset of 4 from position 25 and the source of the offset
 * is not quite clear) are upto 4 groups of 5 characters
 * specifying the elemental composition of the species.  The first two characters name the
 * element, the next three give its count.
 *
 * These notes are based on reading the code below, the data format is taken from Chemkin,
 * which should provide a more authoritative description.  However, this code has not
 * been extensively tested for support of the chemkin data format so it probably does not
 * accept exactly the same inputs as any version of chemkin.
 */
