		...
		while (m_t < dt && m_PAH.size() > 1)
		{
			...
			if (totalsites > 1 && numdiffPAHs > 1){
				...
				if (m_t < dt)
				{
					...
					size_t ip1 = chooseIndex<double>(Rates, uniformGenerator);
					size_t ip2 = ip1;
...
int PAHPrimary::NumRings5Lone() const
{
	return m_numOfRings5Lone;
}
