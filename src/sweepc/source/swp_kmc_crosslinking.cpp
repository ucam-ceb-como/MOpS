/*
  Author(s):      Gustavo Leon
  Project:        sweepc (population balance solver)
  Sourceforge:    http://sourceforge.net/projects/mopssuite

  Copyright (C) 2019 Gustavo Leon.

  File purpose:
    Implementation of the crosslinking coagulation kernel
...
*/
#include "swp_kmc_crosslinking.h"
...
//Calculated by Angiras Menon for the reaction A2R5- + A2 = A2--A2R5 
//The reaction between a zig-zag radical with a zig-zag PAH
const double Sweep::Processes::Crosslinking::s_Arrhenius_factor = 4.79504;    //cm^3/(mol*s)
const double Sweep::Processes::Crosslinking::s_T_dependance = 3.13411;
const double Sweep::Processes::Crosslinking::s_Activation_Energy = 11.2745;			//kJ/mol

// Default constructor.
Sweep::Processes::CrosslinkingCoagulation::CrosslinkingCoagulation(const Sweep::Mechanism &mech)
: Coagulation(mech), m_efm(mech.GetEnhancementFM())
{
    m_name = "CrosslinkingCoagulation";
}

Sweep::Processes::CrosslinkingCoagulation* const Sweep::Processes::CrosslinkingCoagulation::Clone() const
{
    return new CrosslinkingCoagulation(*this);
}

// Stream-reading constructor.
Sweep::Processes::CrosslinkingCoagulation::CrosslinkingCoagulation(std::istream &in, const Sweep::Mechanism &mech)
: Coagulation(mech), m_efm(mech.GetEnhancementFM())
{
    m_name = "CrosslinkingCoagulation";
    Deserialize(in, mech);
...
double Sweep::Processes::CrosslinkingCoagulation::Rate(double t, const Cell &sys,
...
// as arguments.
double Sweep::Processes::CrosslinkingCoagulation::Rate(const Ensemble::particle_cache_type &data, double n, double sqrtT,
...
unsigned int Sweep::Processes::CrosslinkingCoagulation::TermCount(void) const {return TYPE_COUNT;}
 ...
 */
double Sweep::Processes::CrosslinkingCoagulation::RateTerms(double t, const Cell &sys,
...
// passed as arguments.
double Sweep::Processes::CrosslinkingCoagulation::RateTerms(const Ensemble::particle_cache_type &data, double n, double sqrtT,
 ...
 */
int CrosslinkingCoagulation::Perform(double t, Sweep::Cell &sys, 
                                   const Geometry::LocalGeometry1d& local_geom,
                                   unsigned int iterm,
 ...
 */
double Sweep::Processes::CrosslinkingCoagulation::CoagKernel(const Particle &sp1,
                                                                const Particle &sp2,
 ...
 *@return       Value of kernel
 */
double Sweep::Processes::CrosslinkingCoagulation::MajorantKernel(const Particle &sp1,
                                                                    const Particle &sp2,
                                                                    const Cell &sys,
                                                                    const MajorantType maj) const
{
    ...
    switch (maj) {
        case Default:
            // This should never happen for the crosslinking coagulation kernel
            assert(maj != Default);
            break;
        case FreeMol:
...
// true kernel.
double Sweep::Processes::CrosslinkingCoagulation::FreeMolKernel(const Particle &sp1, const Particle &sp2,
...
// two given particles.  Can return either the majorant or
// true kernel.
double Sweep::Processes::CrosslinkingCoagulation::SlipFlowKernel(const Particle &sp1, const Particle &sp2,
