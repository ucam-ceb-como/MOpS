...
cpair PAHProcess::jumpToPos(const cpair& starting, const angletype& direction, const angletype& direction2, const bondlength& distance) const{
	...
	auto temp = std::make_tuple(std::get<0>(starting) +distance*x_inc(direction), std::get<1>(starting) +distance*y_inc(direction), std::get<2>(starting) +distance*z_inc(direction2));
...
void PAHProcess::checkHindrance_C_intPAH(cpair coords) const {
	double tol = 1e-3;
...
bool PAHProcess::checkHindrancePhenyl(const Cpointer C_1) const {
	...
	mpos = jumpToPos(C_1->coords, C_1->bondAngle2, 0, 1); // position O1
...
Spointer PAHProcess::RZZchooseRandomSite(kmcSiteType st, rng_type &rng) {
	// to choose any principal site
		//cout << "~~Choosing from " << m_pah->m_siteMap[st].size() << " sites...\n";
	int R5_unavail_sites = (int)m_pah->m_siteMap[R5R6].size() + (int)m_pah->m_siteMap[R5R6_emb].size() + (int)m_pah->m_siteMap[BY5R5].size() + (int)m_pah->m_siteMap[BY6R5].size();
	if (R5_unavail_sites > 0){
		...
		for (int i = 0; i < (int)m_pah->m_siteMap[st].size(); i++) {
			...
			if (S1->type != R5R6 && S2->type != R5R6 && S1->type != R5R6_emb && S2->type != R5R6_emb && S1->type != BY5R5 && S2->type != BY5R5 && S1->type != BY6R5 && S2->type != BY6R5){
...
void PAHProcess::moveC(Cpointer C_1, cpair newpos) {
	...
	auto temp = std::make_tuple(std::get<0>(newpos), std::get<1>(newpos), std::get<2>(newpos));
...
void PAHProcess::moveC(Cpointer C_1, Cpointer Cprev, double new_distance) {
	...
	auto temp = std::make_tuple(std::get<0>(Cprev->coords) + (std::get<0>(C_1->coords) - std::get<0>(Cprev->coords)) / dist*new_distance, std::get<1>(Cprev->coords) + (std::get<1>(C_1->coords) - std::get<1>(Cprev->coords)) / dist*new_distance, std::get<2>(Cprev->coords) + (std::get<2>(C_1->coords) - std::get<2>(Cprev->coords)) / dist*new_distance);
	/*C_1->coords.first = newpos.first;
	C_1->coords.second = newpos.second;*/
	C_1->coords = temp;
	m_pah->m_cpositions.insert(C_1->coords);
...
void PAHProcess::includeCurvature(cpair C_1, cpair C_2, cpair C_3){
	//C_1 and C_2 are the R5 positions before bay capping. Before the bond was 3^0.5 long and is rescaled to length 1. 
	...
	double Bx = (std::get<0>(C_3) -std::get<0>(C_1)); //x component of B
	double By = (std::get<1>(C_3) -std::get<1>(C_1)); //y component of B
	double Bz = (std::get<2>(C_3) -std::get<2>(C_1)); //z component of B

	double C_x1, C_y1, C_x2, C_y2, C_z1, C_z2, R5centre_x, R5centre_y, R5centre_z;
	C_x1 = (std::get<0>(C_2) +std::get<0>(C_1)) / 2.0 - Ax / 2.0;
	C_x2 = (std::get<0>(C_2) +std::get<0>(C_1)) / 2.0 + Ax / 2.0;
	C_y1 = (std::get<1>(C_2) +std::get<1>(C_1)) / 2.0 - Ay / 2.0;
	C_y2 = (std::get<1>(C_2) +std::get<1>(C_1)) / 2.0 + Ay / 2.0;
	C_z1 = (std::get<2>(C_2) +std::get<2>(C_1)) / 2.0 - Az / 2.0;
	C_z2 = (std::get<2>(C_2) +std::get<2>(C_1)) / 2.0 + Az / 2.0;


	//R5centre is the position of the just encapped 5 member ring
	//normal components
	double nx = (Ay*Bz - Az*By);
	double ny = (Az*Bx - Ax*Bz);
	double nz = (Ax*By - Ay*Bx);

	Ax = C_x2 - ((std::get<0>(C_2) +std::get<0>(C_1)) / 2.0);
	Ay = C_y2 - ((std::get<1>(C_2) +std::get<1>(C_1)) / 2.0);
	Az = C_z2 - ((std::get<2>(C_2) +std::get<2>(C_1)) / 2.0);

	double R5x = (Ay*nz - Az*ny);
	double R5y = (Az*nx - Ax*nz);
	double R5z = (Ax*ny - Ay*nx);
	double magnitude = pow(pow(R5x, 2.0) + pow(R5y, 2.0) + pow(R5z, 2.0), 0.5);

	R5x = R5x / magnitude;
	R5y = R5y / magnitude;
	R5z = R5z / magnitude;
	double distance = (sin(54.0 * M_PI / 180.0) / (2.0*sin(36.0 * M_PI / 180.0)));

	R5centre_x = ((std::get<0>(C_2) +std::get<0>(C_1)) / 2.0) + R5x*distance;
	R5centre_y = ((std::get<1>(C_2) +std::get<1>(C_1)) / 2.0) + R5y*distance;
	R5centre_z = ((std::get<2>(C_2) +std::get<2>(C_1)) / 2.0) + R5z*distance;
	
	Ccontainer::iterator it;
	for (it = m_pah->m_carbonList.begin(); it != m_pah->m_carbonList.end(); ++it) {
		double Cnew_x, Cnew_y, Cnew_z, Cdistance;
		Cpointer C_change = *it;
		Cdistance = pow(pow((std::get<0>(C_change->coords) - R5centre_x), 2) + pow((std::get<1>(C_change->coords) - R5centre_y), 2) + pow(std::get<2>(C_change->coords) - R5centre_z,2), 0.5);
		Cnew_x = R5centre_x + cos(30.0*M_PI / 180.0)*(std::get<0>(C_change->coords) - R5centre_x);
		Cnew_y = R5centre_y + cos(30.0*M_PI / 180.0)*(std::get<1>(C_change->coords) - R5centre_y);
		Cnew_z = std::get<2>(C_change->coords) + Cdistance *sin(30.0*M_PI / 180.0);
		auto temp = std::make_tuple(Cnew_x, Cnew_y, Cnew_z);
	...
	for (it2 = m_pah->m_InternalCarbons.begin(); it2 != m_pah->m_InternalCarbons.end(); ++it2){
		double Cnew_x, Cnew_y, Cnew_z, Cdistance;
		Cdistance = pow(pow((std::get<0>(*it2) - R5centre_x), 2) + pow((std::get<1>(*it2) - R5centre_y), 2) + pow(std::get<2>(*it2) - R5centre_z, 2), 0.5);
		Cnew_x = R5centre_x + cos(30.0*M_PI / 180.0)*(std::get<0>(*it2) - R5centre_x);
		Cnew_y = R5centre_y + cos(30.0*M_PI / 180.0)*(std::get<1>(*it2) - R5centre_y);
		Cnew_z = std::get<2>(*it2) + Cdistance *sin(30.0*M_PI / 180.0);
		auto temp = std::make_tuple(Cnew_x, Cnew_y, Cnew_z);
		cpair & s(*it2);
		s = temp;
		int test = 1;
...
Cpointer PAHProcess::bridgeC(Cpointer C_1) {
    ...
    cb->bridge = true;
    // set new coordinates and store
    cb->coords = jumpToPos(C_1->coords, C_1->bondAngle2, 0, 1);
    if(!m_pah->m_carbonList.insert(cb).second)
        std::cout<<"ERROR: ADDING SAME CARBON POINTER TO SET\n";
    m_pah->m_cpositions.insert(cb->coords);
    // Set details of C_1
...
void PAHProcess::addR5toSite(Spointer& st, Cpointer Carb1, Cpointer Carb2) {
	...
	if ((kmcSiteType)stype == None) {
		...
		saveDOT("KMC_DEBUG/add5RtoSite_error.dot");
		return;
	}
...
void PAHProcess::remR5fromSite(Spointer& st, Cpointer Carb1, Cpointer Carb2) {
	...
	if ((kmcSiteType)stype == None) {
		...
		saveDOT("KMC_DEBUG/rem5RfromSite_error.dot");
		return;
	}
	// removes site from m_pah->m_siteMap (principal site)
	delSiteFromMap(st->type, st);
    ...
    if(stype<13 && stype>9) // i.e. principal sites with 5R at both sides (eg RFER)
		...
		stype -= 6;
	else if (stype > 20 && stype < 24) //i.e. principal site is R5R6_emb, BY5R5 or BY6R5 and are losing one carbon atom from the structure only
		stype -= 1;
...
void PAHProcess::redrawR5(Spointer& st, Cpointer Carb1, Cpointer Carb2) {
	...
	cpair mpos = jumpToPos(Carb1->coords, normAngle(Carb1->C1->bondAngle1 - 60), 0, 1);
	if (!checkHindrance_C_PAH(mpos)) {
		addC(Carb1, normAngle(Carb1->C1->bondAngle1 - 60), normAngle(Carb1->C1->bondAngle1), 1, true);
		...
		C1_res = addC(Carb1, normAngle(Carb1->bondAngle1 + 120), 0, 1);
		C2_res = addC(C1_res, normAngle(Carb1->bondAngle1 - 90), normAngle(Carb1->bondAngle1 - 180), pow(3, 0.5));
...
void PAHProcess::convSiteType(Spointer& st, Cpointer Carb1, Cpointer Carb2, kmcSiteType t) {
	...
	if (!checkSiteValid(st)) {
		cout << "Invalid site. This may be alright if the edge is unreactive but it may also be an error. \n";
	}
    // add site to m_pah->m_siteMap
...
void PAHProcess::updateSites(Spointer& st, // site to be updated
	...
	/*
	if (abs(bulkCchange) < 5 ){ //Site is now neighbour to a R5
		if (stype < 6) {
	...
	/*
	if (stype == 20){ //The neighbour site is an embedded or partially embedded R5
		// removes site from m_pah->m_siteMap (principal site)
		delSiteFromMap(st->type, st);
		// change site type
...
PAHStructure& PAHProcess::initialise(StartingStructure ss){
    ...
    case BENZENE_C:
        ...
        newC = addC(m_pah->m_cfirst, 0, 0, 1);
        //cout << "newC is " << newC << '\n';
        newC = addC(newC, -60, 0, 1);
        //cout << "newC is " << newC << '\n';
        newC = addC(newC, -120, 0, 1);
        //cout << "newC is " << newC << '\n';
        newC = addC(newC, -180, 0, 1);
        //cout << "newC is " << newC << '\n';
        // adds the last C atom, with bond angle towards m_cfirst
        m_pah->m_clast = addC(newC, 120, 60, 1);
    ...
    case PYRENE_C:
		...
		newC = addC(newC, 60, 0, 1);
        // adds the last C atom, with bond angle towards m_cfirst
		m_pah->m_clast = addC(newC, 0, 60, 1);
        // closes structure
        connectToC(m_pah->m_clast, m_pah->m_cfirst);
        // update H atoms
        updateA(m_pah->m_cfirst, m_pah->m_clast, 'H');
...
Cpointer PAHProcess::drawType0Site(Cpointer Cnow, int bulkC) {
    ...
    for(int c=0; c<=bulkC; ++c) {
        ...
        cpair pos = jumpToPos(Cnow->coords, angle, 0, 1);
        if(m_pah->m_cpositions.count(pos)) {
            ...
            Cnow = addC(Cnow, angle, angle, 1, false);
...
Cpointer PAHProcess::drawType1Site(Cpointer Cnow, int bulkC, kmcSiteType prevType) {
    ...
    if(bulkC == 0) {
        ...
        else Cnow = addC(Cnow, angle, normAngle(angle-30), 1, false);
        return Cnow;
    }else { //draw RXX site
    ...
    do{
        ...
        if(next->bridge) {// if next C is a bridge
            if(now == next->C1) { // check if current C is next->C1
                corr_coords = jumpToPos(now->coords, now->bondAngle1, 0, 1);
                next = next->C3; // cross bridge
            }else if(now == next->C3) { // check if current C is from bridge
                corr_coords = jumpToPos(now->coords, now->bondAngle2, 0, 1);
                next = next->C2;
            }
...
bool PAHProcess::performProcess(const JumpProcess& jp, rng_type &rng, int PAH_ID)
{
    ...
    switch(id) {
		...
		case 35:
			proc_GR7_BY5R5(site_perf, site_C1, site_C2, rng); break;
        default:
...
void PAHProcess::proc_G6R_AC(Spointer& stt, Cpointer C_1, Cpointer C_2) {
    ...
    if(!(C_1->C2->bridge) || !(C_2->C1->bridge)) { // check if bulk C in AC site is a bridge
        ...
        removeC(C_1->C2, true);
        removeC(C_2->C1, true);
        newC1 = addC(C_1, normAngle(C_1->bondAngle1+120), 0, 1);
        newC2 = addC(newC1, normAngle(newC1->C1->bondAngle1-60), normAngle(newC1->C1->bondAngle1-120), 1);
    }else {
	...
	Spointer S5 = moveIt(S1, -1);
	Spointer S6 = moveIt(S2, 1);
	...
	//Check for R5R6 sites and embed them
	/*if ((int)S5->type == 20){
		convSiteType(S5, S5->C1, S5->C2, (kmcSiteType)21);
	}
	if ((int)S6->type == 20){
		convSiteType(S6, S6->C1, S6->C2, (kmcSiteType)21);
	}*/
...
//    printSites(stt);
    ...
    // Add C
	newC1 = addC(C_1, normAngle(C_1->bondAngle1 + 120), 0, 1);
	newC2 = addC(newC1, normAngle(newC1->C1->bondAngle1 - 60), 0, 1);
	newC3 = addC(newC2, normAngle(newC2->C1->bondAngle1 - 60), 0, 1);
	newC4 = addC(newC3, normAngle(newC3->C1->bondAngle1 - 60), normAngle(newC3->C1->bondAngle1 - 120), 1);
...
void PAHProcess::proc_L6_BY6(Spointer& stt, Cpointer C_1, Cpointer C_2) {
    ...
    if(ntype1 < 5 && ntype2 < 5) {
		...
		if (ntype_site == 4) newType = (ntype1 + ntype2 + 2);
			...
			new_point = 63;
			if (ntype1 >= 51 && ntype1 <= 54) ntype1 -= 51;
			else if (ntype2 >= 51 && ntype1 <= 54) ntype2 -= 51;
...
void PAHProcess::proc_PH_benz(Spointer& stt, Cpointer C_1, Cpointer C_2, rng_type &rng) {
    ...
    Cpointer newC;
    newC = bridgeC(chosen);
	newC = addC(newC, normAngle(chosen->bondAngle2 + 60), 0, 1);
	newC = addC(newC, normAngle(newC->C1->bondAngle1 - 60), 0, 1);
	newC = addC(newC, normAngle(newC->C1->bondAngle1 - 60), 0, 1);
	newC = addC(newC, normAngle(newC->C1->bondAngle1 - 60), 0, 1);
	newC = addC(newC, normAngle(newC->C1->bondAngle1 - 60), normAngle(newC->C1->bondAngle1 - 120), 1);
...
void PAHProcess::proc_O6R_FE_HACA_O2(Spointer& stt, Cpointer C_1, Cpointer C_2) {
    ...
    cpair pos = jumpToPos(C1_res->coords, normAngle(C1_res->bondAngle1-120), 0, 1);
    ...
    removeC(C_2, false);
	if (bridge){
		for (Ccontainer::iterator it = m_pah->m_carbonList.begin(); it != m_pah->m_carbonList.end(); it++){
			Cpointer C_k = *it;
			double tol = 1e-3;
			if (abs(std::get<0>(C_k->coords) - std::get<0>(pos)) < tol && abs(std::get<1>(C_k->coords) - std::get<1>(pos)) < tol && abs(std::get<2>(C_k->coords) - std::get<2>(pos)) < tol){
		...
		Cpointer Cnew1 = addC(C1_res, normAngle(C1_res->bondAngle1 - 120), 0, 1, true);
		Cpointer Cnew2 = addC(C1_res->C2, normAngle(C1_res->bondAngle1 + 60), normAngle(C1_res->bondAngle1 + 120), 1, true);
		// update H
	...
	if (S1->type == R5R6 || (S2->type == R5R6)) {
		...
		if ((int)S3->type > 500) updateSites(S3, S3->C1, S3->C2, -400);
		if ((int)S4->type > 500) updateSites(S4, S4->C1, S4->C2, -400);
	}
	...
	else if (((int)S1->type >= 63 && (int)S1->type <= 65) && ((int)S2->type >= 63 && (int)S2->type <= 65)){
		double dist1 = getDistance_twoC(C_1->C1, C_1->C1->C1);
		double dist2 = getDistance_twoC(C_2->C2, C_2->C2->C2);
	...
	else if ((int)S1->type >= 63 && (int)S1->type <= 65) {
		double dist2 = getDistance_twoC(C_1->C1, C_1->C1->C1);
		if (dist2 > 1.1){
			updateSites(stt, C1_res, C2_res, 502);
			updateSites(S1, S1->C1, C1_res, -11);
			updateSites(S2, C2_res, S2->C2, -1);
	...
	else if ((int)S2->type >= 63 && (int)S2->type <= 65) {
		double dist2 = getDistance_twoC(C_2->C2, C_2->C2->C2);
		if (dist2 > 1.1){
			updateSites(stt, C1_res, C2_res, 502);
			updateSites(S1, S1->C1, C1_res, -1);
	...
	else if (S1->type == R5R6FE || (S2->type == R5R6FE)) {
		if (S1->type == R5R6FE && (S2->type == R5R6FE)) {
...
void PAHProcess::proc_G5R_ZZ(Spointer& stt, Cpointer C_1, Cpointer C_2) {
    ...
    removeC(C_2->C1, true);
    Cpointer C1_res, C2_res;
    C1_res = addC(C_1, normAngle(C_1->bondAngle1+120), 0, 1);
	C2_res = addC(C1_res, normAngle(C_1->bondAngle1 - 90), normAngle(C_1->bondAngle1 - 180), pow(3, 0.5));
    updateA(C_1->C1, C_2->C2, 'H');
    // update sites and neighbours
...
void PAHProcess::proc_D5R_R5(Spointer& stt, Cpointer C_1, Cpointer C_2) {
	...
	//moveC(C1_res, C1_res->C1, 1);
	//moveC(C2_res, C2_res->C2, 1);
    addC(C1_res, normAngle(C1_res->C1->bondAngle1-60), normAngle(C1_res->C1->bondAngle1), 1, true);
...
void PAHProcess::proc_C6R_AC_FE3(Spointer& stt, Cpointer C_1, Cpointer C_2, rng_type &rng) {
	...
	C1_R5 = addC(Cstart, normAngle(Cstart->bondAngle1 + 120), 0, 1, false);
	C2_R5 = addC(C1_R5, normAngle(Cstart->bondAngle1 - 90), normAngle(Cstart->bondAngle1 - 180), pow(3, 0.5), false);
...
void PAHProcess::proc_C5R_RFE(Spointer& stt, Cpointer C_1, Cpointer C_2) {
	...
	/*double dist_inc = (tan(36 * M_PI / 180) / tan(30 * M_PI / 180) - 1.0) / ((tan(36 * M_PI / 180) / tan(30 * M_PI / 180)) + 1.0);
	moveC(Cstart, Cstart->C1, 1);
	moveC(Cstart->C2->C2->C2, Cstart->C2->C2->C2->C2, 1);*/
    for(int i=0; i!=2; i++) removeC(Cstart->C2, false);
    addC(Cstart, normAngle(Cstart->bondAngle1-120), normAngle(Cstart->bondAngle1-60), 1, true);
    ...
    if(b4) 
        ...
        Cstart = C_1;
    C1_new = addC(Cstart, normAngle(Cstart->bondAngle1+120), 0, 1);
    C2_new = addC(C1_new, normAngle(C1_new->C1->bondAngle1-60), 0, 1);
    C3_new = addC(C2_new, normAngle(C2_new->C1->bondAngle1-60), 0, 1);
    C4_new = addC(C3_new, normAngle(C3_new->C1->bondAngle1-60), normAngle(C3_new->C1->bondAngle1-120), 1);
...
void PAHProcess::proc_C5R_RAC(Spointer& stt, Cpointer C_1, Cpointer C_2) {
    ...
    C1_new = addC(Cstart, normAngle(Cstart->bondAngle1+120), 0, 1);
    C2_new = addC(C1_new, normAngle(C1_new->C1->bondAngle1-60), normAngle(C1_new->C1->bondAngle1-120), 1);
    // edit sites. first identify the neighbouring sites of resulting AC & FE3
    Spointer S1, S2, S3, S4;
    if(b4) {
        S1 = moveIt(sR5, -1); // neighbour of R5
...
void PAHProcess::proc_M5R_RZZ(Spointer& stt, Cpointer C_1, Cpointer C_2) {
	...
	//Check for ZZ site
	cpair mpos = jumpToPos(Ccheck->coords, normAngle(Ccheck->C1->bondAngle1 - 60), 0, 1);
    // remove R5 first, leaving a ZZ site (adding another C atom after removing C)
    for(int i=0; i!=2; i++) removeC(Cstart->C2, false);
	...
	if (!checkHindrance_C_PAH(mpos)) {
		...
		removeC(Cstart->C2, true);
		//Add new R5
		C1_R5 = addC(Cstart, normAngle(Cstart->bondAngle1 + 120), 0, 1);
		C2_R5 = addC(C1_R5, normAngle(C1_R5->C1->bondAngle1 - 90), normAngle(C1_R5->C1->bondAngle1 - 180), pow(3, 0.5));
		// update H atoms
		...
		if (b4){
			...
			//Add new R5
			C1_R5 = addC(Cstart, normAngle(Cstart->bondAngle1), 0, 1);
			C2_R5 = addC(C1_R5, normAngle(C1_R5->C1->bondAngle1 - 90), normAngle(C1_R5->C1->bondAngle1 - 180), pow(3, 0.5));
			//C2_R5->C2 = C_2;
			//Update 'H'
...
void PAHProcess::proc_C6R_BY5_FE3(Spointer& stt, Cpointer C_1, Cpointer C_2, rng_type &rng) {
	...
	// add a C atom
	addC(C_1, normAngle(C_1->bondAngle1 + 120), normAngle(C_1->bondAngle1 + 60), 1);
	/*double dist_inc = (tan(36 * M_PI / 180) / tan(30 * M_PI / 180) - 1.0) / ((tan(36 * M_PI / 180) / tan(30 * M_PI / 180)) + 1.0);
	moveC(CRem->C1->C1, CRem->C1->C1->C1, 1 + dist_inc);*/
	//addC(CRem->C1->C1, normAngle(CRem->C1->C1->bondAngle1 - 6), 0, 1);
	//removeC(CRem->C1, false);
	Cpointer CR5_1 = CRem->C1;
	removeC(CRem, false);
	CR5_1->bondAngle1 = CR5_1->bondAngle1 - 30;
	//addC(CR5_1, normAngle(CR5_1->C1->bondAngle1 - 72), normAngle(CR5_1->C1->bondAngle1 - 144), 1);
	//removeC(CR5_1->C2->C2, false);
	//moveC(CR5_1->C2->C2, CR5_1->C2->C2->C2, 1 + dist_inc);
...
void PAHProcess::proc_L5R_BY5(Spointer& stt, Cpointer C_1, Cpointer C_2) {
	...
	else if (ntype1 == 51 && ntype2 == 51) convSiteType(stt, moveIt(stt, -1)->C1, moveIt(stt, 1)->C2, (kmcSiteType)264);
	else if (ntype1 == 51 || ntype2 == 51){
		if (ntype1 == 51) ntype1 = 0;
		if (ntype2 == 51) ntype2 = 0;
		newType = 163 + ntype1 + ntype2;
		convSiteType(stt, moveIt(stt, -1)->C1, moveIt(stt, 1)->C2, (kmcSiteType)newType);
	}
	else {
		int new_point = 0;
...
void PAHProcess::proc_O6R_FE2(Spointer& stt, Cpointer C_1, Cpointer C_2) {
    ...
    addC(C_start, normAngle(C_start->bondAngle1-120), normAngle(C_start->bondAngle1-60), 1, true);
    // update H
...
void PAHProcess::proc_B6R_ACR5(Spointer& stt, Cpointer C_1, Cpointer C_2) {
	...
	if (!(C_1->C2->bridge) || !(C_2->C1->bridge)) { // check if bulk C in AC site is a bridge
		...
		double dist_inc = (tan(36 * M_PI / 180) / tan(30 * M_PI / 180) - 1.0) / ((tan(36 * M_PI / 180) / tan(30 * M_PI / 180)) + 1.0);
		removeC(C_1->C2, true);
		newC1 = addC(C_1, normAngle(C_1->bondAngle1 - 150), 0, 1 + dist_inc);
		removeC(newC1->C2, true);
		newC2 = addC(newC1, normAngle(newC1->C1->bondAngle1 - 60), normAngle(newC1->C1->bondAngle1 +120), 1.0);
		/*removeC(C_1->C2, true);
		newC1 = addC(C_1, normAngle(C_1->bondAngle1 + 75), 0, 1.5);
		removeC(newC1->C2, true);
		newC2 = addC(newC1, normAngle(newC1->C1->bondAngle1 -45), normAngle(newC1->C1->bondAngle1 - 60), 1.5);*/
	}
	else {
		// update bridges info, both are no longer bridges
		C_1->C2->C1 = C_1->C2->C3;// update neighbour
	...
	//printSites(stt);
	//printStruct();
	//includeCurvature(CR5_1, CR5_2, CR5_3);
}
...
void PAHProcess::proc_M5R_ACR5_ZZ(Spointer& stt, Cpointer C_1, Cpointer C_2, rng_type &rng) {
	...
	if ((int)sFE2->type == 101 || (int)sFE2->type == 501 || (int)sFE2->type == 51) return; // This would violate the IPR.
	if ((int)sFE2->type == 0){
		...
		if ((int)checkR5_1->type == 0){
			if ((int)checkR5_2->type >= 50 && (int)checkR5_2->type <= 65) return;
	...
	if (b4) {
		...
		else if ((int)sFE2->type == 0 && (int)S2->type == 2){ //sFE2 is a FE followed by a ZZ
			//This means that the pentagon has migrated to the edge but will have a carbon out of the structure.
			convSiteType(sFE2, sFE2->C1, sFE2->C2, R5R6);
			updateSites(S2, sFE2->C2, S2->C2, 500);
		}
...
void PAHProcess::proc_G6R_RZZ(Spointer& stt, Cpointer C_1, Cpointer C_2) {
	...
	else if(S1->type == R5R6_emb){
		convSiteType(S1, S1->C1, S1->C2->C2, BY5R5); // Converts R5R6 to BY5R5}
		updateSites(S2, newC2, S2->C2, 1); 
	}
	...
	else if (S2->type == R5R6_emb){
		convSiteType(S2, S2->C1->C1, S2->C2, BY5R5); // Converts R5 to BY5R5}
		updateSites(S1, S1->C1, newC1, 1);
	}
...
// ************************************************************
void PAHProcess::proc_GR7_BY5R5(Spointer& stt, Cpointer C_1, Cpointer C_2, rng_type &rng) {
	//printSitesMemb(stt);
	//printStruct();//++++
	Cpointer newC1;
	Cpointer newC2;
	...
	if (!(C_1->C2->bridge) || !(C_2->C1->bridge)) { // check if bulk C in AC site is a bridge
		...
		newC2 = addC(newC1, normAngle(newC1->C1->C1->bondAngle1 - 30), normAngle(newC1->C1->C1->bondAngle1 - 90), 2.0);
	}
	else {
		// update bridges info, both are no longer bridges
		C_1->C2->C1 = C_1->C2->C3;// update neighbour
		...
		connectToC(C_1, C_2);
		// Add C
		newC1 = addC(C_1, normAngle(C_1->bondAngle1 + 90), 0, 1.5);
		newC2 = addC(newC1, normAngle(newC1->C1->bondAngle1 - 60), normAngle(newC1->C1->bondAngle1 - 90), 1.5);
	}
	//printStruct();
	// Add and remove H
	updateA(C_1->C1, C_2->C2, 'H');
	...
	if (S1->type == R5) convSiteType(S1, S1->C1, newC1, R5R6); // neighbours
	else updateSites(S1, S1->C1, newC1, 1); // neighbours
	if (S2->type == R5) convSiteType(S2, newC2, S2->C2, R5R6);
