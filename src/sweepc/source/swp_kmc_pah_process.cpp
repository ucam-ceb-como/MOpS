...
void PAHProcess::save_trajectory_xyz(const double timer, const std::string &filename, bool optimise) {
	...
	for (int i=1; i!=25; ++i)	conv.Write(&mol, &ofs1);
	ofs1.close();
}
...
cpair PAHProcess::scale_vector(cpair vec) const{
	...
	{
		double magnitude = pow(std::get<0>(vec)*std::get<0>(vec) + std::get<1>(vec)*std::get<1>(vec) + std::get<2>(vec)*std::get<2>(vec), 0.5);
...
OpenBabel::OBMol PAHProcess::passPAH(bool detectBonds) {
	...
	if (detectBonds){
		...
		for (OpenBabel::OBBondIterator bond_iter=mol.BeginBonds(); bond_iter != mol.EndBonds(); bond_iter++){
			...
			if (length_bond >= 1.75) bonds_to_delete.push_back( (*bond_iter)->GetIdx() );
		}
...
OpenBabel::OBMol PAHProcess::optimisePAH(OpenBabel::OBMol mol, int nsteps, std::string forcefield) {
	...
	if (!pFF->Setup(mol)) {
	  ...
	  saveXYZ("KMC_DEBUG/Forcefield_error");
      exit (-1);
    }
	bool done = true;
...
void PAHProcess::redrawR5(Spointer& st, Cpointer Carb1, Cpointer Carb2) {
	...
	cpair normvec = norm_vector(Carb1->coords, Carb2->coords, Carb1->C1->coords);
...
void PAHProcess::updateSites(Spointer& st, // site to be updated
	...
	if (!checkSiteValid(stype)){
		...
		saveXYZ("KMC_DEBUG/KMC_PAH_X_UPDATE_prev");
		throw std::runtime_error(msg.str());
		assert(false);
		return;
	}
	...
	st->C1 = Carb1;
	st->C2 = Carb2;
	//Adjust growth vector to avoid steric hindrance
	/*if( (int)st->type%10 >= 4 ){
		cpair vec1 = get_vector(Carb1->C1->coords,Carb1->coords);
		cpair vec2 = get_vector(Carb1->coords,Carb1->C2->coords);
		cpair normvec = cross_vector(vec1, vec2); 
		Carb1->growth_vector = normvec;
		vec1 = get_vector(Carb2->C1->coords,Carb2->coords);
		vec2 = get_vector(Carb2->coords,Carb2->C2->coords);
		normvec = cross_vector(vec2, vec1);
		Carb2->growth_vector = normvec;
	}*/
...
Cpointer PAHProcess::findThirdC(Cpointer C_1) {
	for(Ccontainer::iterator i=m_pah->m_carbonList.begin(); i != m_pah->m_carbonList.end(); ++i) {
		Cpointer C_check = *i;
		if (C_check != C_1 && C_check != C_1->C1 && C_check != C_1->C2){
...
bool PAHProcess::performProcess(const JumpProcess& jp, rng_type &rng, int PAH_ID)
{
    ...
    switch(id) {
		...
		case 35:
			proc_GR7_BY5R5(site_perf, site_C1, site_C2); break;
		case 36:
			proc_GR7_R6ACR5(site_perf, site_C1, site_C2); break;
        default:
	...
	if (!checkSiteValid(S1) || !checkSiteValid(S2) || !checkSiteValid(S3) || !checkSiteValid(S4)){
		...
		saveXYZ("KMC_DEBUG/checkSiteValid_PerformProcess");
		std::ostringstream msg;
    ...
    if(!checkCombinedSiteType(site_perf) || !checkCombinedSiteType(S1)
        ...
        std::cout<<"Structure produced invalid combined site type after performing process "
            ...
            <<"*************\nAfter performing process --\n";
        printSites(site_perf);
		saveXYZ("KMC_DEBUG/checkCombinedSite");
        std::ostringstream msg;
        msg << "ERROR: Structure produced invalid combined site type after performing process "
            << "ID"<<id<<" on PAH ID: "<< PAH_ID <<"..."
            << " (Sweep::KMC_ARS::PAHProcess::performProcess)";
	...
	if(calc_total != getCHCount().first) {
		...
		saveXYZ("KMC_DEBUG/KMC_C_Counts_ERROR");
	...
	if (id == 23){
		ofstream ofs1;
		std::string filename1 = "KMC_DEBUG/R5embedding.txt";
		ofs1.open(filename1, std::ios::app);
		ofs1<<PAH_ID<<"\n";
		ofs1.close();
	}
	if (id == 35 || id == 36){
		ofstream ofs1;
		std::string filename1 = "KMC_DEBUG/R7.txt";
		ofs1.open(filename1, std::ios::app);
		ofs1<<PAH_ID<<"\n";
		ofs1.close();
	}
...
void PAHProcess::proc_G6R_AC(Spointer& stt, Cpointer C_1, Cpointer C_2) {
    ...
    if(!(C_1->C2->bridge) || !(C_2->C1->bridge)) { // check if bulk C in AC site is a bridge
		...
		updateA(C_1, 'C', C_1->growth_vector);
		newC2 = addC(newC1, lat_direction, 1.4);
	...
	if (!hept_bool){
		if (S1->type == R5R6 || (S2->type == R5R6)) {
			...
			if (S1->type == R5R6FE && (S2->type == R5R6FE)) {
				...
				convSiteType(stt, C1_res, C2_res, RZZ);
				S4 = moveIt(S2, +1);
				convSiteType(S4, S4->C1, S4->C2, RFE);
		...
		if (S1->type == ACR5 && S2->type == ACR5) {
			updateSites(stt, C1_res, C2_res, 264);
			updateSites(S1, S1->C1, C1_res, -11);
			updateSites(S2, C2_res, S2->C2, -11);
		}
		else if (S1->type == ACR5 || S2->type == ACR5) {
			updateSites(stt, C1_res, C2_res, 163);
			updateSites(S1, S1->C1, C1_res, -1);
			updateSites(S2, C2_res, S2->C2, -1);
		}
		else if (((int)S1->type >= 63 && (int)S1->type <= 65) && ((int)S2->type >= 63 && (int)S2->type <= 65)){
			...
			double dist2 = getDistance_twoC(C_2->C2, C_2->C2->C2);
			if (dist1 > 1.1 && dist2 > 1.1){
				updateSites(stt, C1_res, C2_res, 62);
				updateSites(S1, S1->C1, C1_res, -1);
				updateSites(S2, C2_res, S2->C2, -1);
			}
			else if (dist1 > 1.1){
			...
			updateSites(stt, C1_res, C2_res, 63);
			updateSites(S1, S1->C1, C1_res, -1);
			updateSites(S2, C2_res, S2->C2, -1);
		}
		// update combined sites
...
void PAHProcess::proc_G5R_ZZ(Spointer& stt, Cpointer C_1, Cpointer C_2) {
	...
	cpair Hdir2 = get_vector(C_1->C2->coords, C_2->coords);
	cpair Cdir = add_vector(get_vector(C_1->coords, C_1->C2->coords),Hdir2);
    removeC(C_2->C1, true);
    Cpointer C1_res, C2_res;
	C1_res = addC(C_1, C_1->growth_vector, 1.4);
	updateA(C1_res, 'H', Hdir1);
	updateA(C_1, 'C', Hdir1);
	C2_res = addC(C1_res, Cdir, 1.4*pow(3, 0.5));
	updateA(C2_res, 'H', Hdir2);
	updateA(C_2, 'C', Hdir1);
...
void PAHProcess::proc_D5R_R5(Spointer& stt, Cpointer C_1, Cpointer C_2) {
    ...
    C1_res = C_1->C1;
    C2_res = C_2->C2;
	cpair Cdir = invert_vector(C_1->growth_vector);
	cpair Hdir1 = get_vector(C1_res->coords, C_1->coords);
	cpair Hdir2 = get_vector(C2_res->coords, C_2->coords);
    removeC(C_1, false);
    removeC(C_2, false);
	...
	Cpointer newC = addC(C1_res, Cdir, 1.4, true);
	updateA(C1_res, 'H', Hdir1);
	updateA(C2_res, 'H', Hdir2);
    /*addC(C1_res, normAngle(C1_res->C1->bondAngle1-60), normAngle(C1_res->C1->bondAngle1), 1.4, true);
...
void PAHProcess::proc_C5R_RFE(Spointer& stt, Cpointer C_1, Cpointer C_2) {
    ...
    if(b4) {
        Cstart = C_2->C1;
		C1_new = addC(Cstart, R5dir, 1.4);
		updateA(C1_new, 'H', Hdir1);
		C2_new = addC(C1_new, Hdir2, 1.4);
		updateA(C2_new, 'H', R5dir);
		C3_new = addC(C2_new, CZZdir, 1.4);
		updateA(C3_new, 'H', Hdir2);
		C4_new = addC(C3_new, invert_vector(R5dir), 1.4);
		updateA(C4_new, 'H', CZZdir);
		updateA(C_2, 'C', Hdir1);
	}
    else {
        Cstart = C_1;
		C1_new = addC(Cstart, Hdir1, 1.4);
		updateA(C1_new, 'H', invert_vector(Hdir2));
		C2_new = addC(C1_new, R5dir, 1.4);
		updateA(C2_new, 'H', Hdir1);
		C3_new = addC(C2_new, Hdir2, 1.4);
		updateA(C3_new, 'H', R5dir);
		C4_new = addC(C3_new, CZZdir, 1.4);
		updateA(C4_new, 'H', Hdir2);
		updateA(C_1, 'C', Hdir2);
	}
...
void PAHProcess::proc_C5R_RAC(Spointer& stt, Cpointer C_1, Cpointer C_2) {
	...
	cpair Hdir2 = sR5->C2->growth_vector;
	cpair CZZdir = invert_vector(Hdir1);
	cpair Cdir = get_vector(Cstart->coords, Cstart->C2->coords);
    for(int i=0; i!=2; i++) removeC(Cstart->C2, false);
	addC(Cstart, CZZdir, 1.4, true);
	updateA(C_AC, 'H', Cdir);
    //addC(Cstart, normAngle(Cstart->bondAngle1-120), normAngle(Cstart->bondAngle1-60), 1.4, true);
    ...
    if(b4) {
		C1_new = addC(Cstart, Cdir, 1.4);
		updateA(C1_new, 'H', Hdir1);
		C2_new = addC(C1_new, Hdir2, 1.4);
		updateA(C2_new, 'H', Cdir);
		updateA(C_2, 'C', Cdir);
	}
    else {
		C1_new = addC(Cstart, Hdir2, 1.4);
		updateA(C1_new, 'H', Cdir);
		C2_new = addC(C1_new, CZZdir, 1.4);
		updateA(C2_new, 'H', Hdir2);
		updateA(C_1, 'C', Cdir);
	}
...
void PAHProcess::proc_L5R_BY5(Spointer& stt, Cpointer C_1, Cpointer C_2) {
	...
	if (ntype1 < 5 && ntype2 < 5) {
		...
		if (ntype1 == 501 && ntype2 == 501) convSiteType(stt, moveIt(stt, -1)->C1, moveIt(stt, 1)->C2, (kmcSiteType)264);
		else {
			ntype1 = ntype1 % 501;
			ntype2 = ntype2 % 501;
			newType = 163 + ntype1 + ntype2;
	...
	else if ( (ntype1 >= 62 && ntype1 <= 63) || (ntype2 >= 62 && ntype2 <= 63) ){
		ntype1 = ntype1 % 62;
		ntype2 = ntype2 % 62;
		newType = 164 + ntype1 + ntype2;
		convSiteType(stt, moveIt(stt, -1)->C1, moveIt(stt, 1)->C2, (kmcSiteType)newType);
	}
	else {
		int new_point = 0;
...
void PAHProcess::proc_M6R_BY5_FE3(Spointer& stt, Cpointer C_1, Cpointer C_2, rng_type &rng) {
    ...
    if(b4) {
		...
		if( (int)S1->type >= 51 && (int) S1->type <= 54) updateSites(sFE3, Cstart, C_1, +500); // convert FE3 to R5R6FE
    } else {
        S1 = moveIt(stt, -1); // neighbour of BY5 (stt)
        S2 = moveIt(sFE3, 1); // neighbour of FE3
		...
		if((int)S2->type >= 51 && (int)S2->type <= 54) updateSites(sFE3, Cstart, C_2->C2->C2, +500); // convert FE3 to R5R6FE
    }
...
void PAHProcess::proc_B6R_ACR5(Spointer& stt, Cpointer C_1, Cpointer C_2) {
	...
	if (!(C_1->C2->bridge) || !(C_2->C1->bridge)) { // check if bulk C in AC site is a bridge
		...
		Hdir1 = C_2->growth_vector;
		Hdir2 = C_1->growth_vector;
		vec1 = get_vector(C_2->C1->coords, C_2->coords);
		vec2 = get_vector(C_1->C2->coords, C_2->C1->coords);
...
void PAHProcess::proc_M5R_ACR5_ZZ(Spointer& stt, Cpointer C_1, Cpointer C_2, rng_type &rng) {
	...
	if (thirdC_after != NULLC){
		...
		if (os_endtype >= 50 && os_endtype <= 65) return;
		if (os_endtype >= 163 && os_endtype <= 165) return;
		if (os_endtype >= 264 && os_endtype <= 265) return;
	}
	...
	if (R5_dist < 1.6){
		...
		saveXYZ("KMC_DEBUG/before_minimisation_shortACR5");
		optimised = true;
		OpenBabel::OBMol mol = passPAH();
		mol = optimisePAH(mol);
		passbackPAH(mol);
		saveXYZ("KMC_DEBUG/after_minimisation_shortACR5");
		//bond_distance = 1.8;
		//return;
	}
	...
	cpair crossvec = cross_vector(normvec, R5dir);
	double theta = asin(R5_dist/2.0/1.4);
	double magn = 1.4 * cos(theta);
	...
	if (optimised){
		//The ACR5 site is on the "short" side of an R5. 
		saveXYZ("before_newcarbons_shortACR5");
		OpenBabel::OBMol newmol = passPAH();
		newmol = optimisePAH(newmol, 4000);
		passbackPAH(newmol);
		saveXYZ("after_newcarbons_shortACR5");
		//return;
	}
	...
	if (opp_site_bool && !opp_site_bool_second && !opp_site_bool_after) {
		updateSites(opp_site, opp_site->C1, opp_site->C2, -60);
		...
		else if ((int)opp_site_after->type >= 1000) updateSites(opp_site_after, opp_site_after->C1, opp_site_after->C2, -800);
		updateSites(opp_site_after, opp_site_after->C1, opp_site_after->C2, +60);
		updateCombinedSites(opp_site);
		updateCombinedSites(opp_site_after);
	}
	else if (opp_site_bool && opp_site_bool_second && !opp_site_bool_after) {
		updateSites(opp_site, opp_site->C1, opp_site->C2, -50);
		updateSites(opp_site_second, opp_site_second->C1, opp_site_second->C2, +10);
		updateCombinedSites(opp_site);
		...
		updateSites(opp_site_second, opp_site_second->C1, opp_site_second->C2, -10);
		updateSites(opp_site_after, opp_site_after->C1, opp_site_after->C2, +50);
		updateCombinedSites(opp_site_second);
		updateCombinedSites(opp_site_after);
	}
	else if (!opp_site_bool && !opp_site_bool_second && opp_site_bool_after) {
...
void PAHProcess::proc_G6R_RZZ(Spointer& stt, Cpointer C_1, Cpointer C_2) {
	...
	if (S1->type == R5){ 
		...
		if (stype >=154 && stype <= 163) stype += 0;
		else stype += 400;
...
void PAHProcess::proc_MR5_R6(Spointer& stt, Cpointer C_1, Cpointer C_2, rng_type &rng) {
	...
	if (b4) {
		...
		int stype;
		if ( (int)S2->type >= 163 && (int)S2->type <= 265 ) stype = S2->type - 100;
		...
		*/
		...
		int stype;
		if ( (int)S1->type >= 163 && (int)S1->type <= 265 ) stype = S1->type - 100;
...
// ************************************************************
void PAHProcess::proc_GR7_BY5R5(Spointer& stt, Cpointer C_1, Cpointer C_2) {
	//printSitesMemb(stt);
	//printStruct();//++++
	OpenBabel::OBMol mol = passPAH();
	mol = optimisePAH(mol);
	...
	if (!(C_1->C2->bridge) || !(C_2->C1->bridge)) { // check if bulk C in AC site is a bridge
		...
		// update bridges info, both are no longer bridges
		C_1->C2->C1 = C_1->C2->C3;// update neighbour
		C_2->C1->C1->C2 = C_2->C1->C1->C3;
		C_1->C2->bridge = false;
		C_2->C1->C1->bridge = false;
		//angletype a = C_2->C1->bondAngle1;
		//C_2->C1->bondAngle1 = C_2->C1->bondAngle2;
		//C_2->C1->bondAngle2 = a;
		//C_1->C2->C3 = NULL;
	...
	else if ( (int)S1->type >= 51 && (int)S1->type <= 54){ 
		int stype = (int)S1->type;
		stype = (stype % 10) + 61;
		convSiteType(S1, S1->C1, newC1, (kmcSiteType)stype); // neighbours
	}	
	...
	else if ( (int)S2->type >= 51 && (int)S2->type <= 54){ 
		int stype = (int)S2->type;
		stype = (stype % 10) + 61;
		convSiteType(S2, newC2, S2->C2, (kmcSiteType)stype); // neighbours
	}	
...
// ID35- R7 growth on embedded-obstructed R5
// ************************************************************
void PAHProcess::proc_GR7_R6ACR5(Spointer& stt, Cpointer C_1, Cpointer C_2) {
	proc_GR7_BY5R5(stt, C_1, C_2);
}
