...
bool PAHProcess::isR5internal(Cpointer C_1, Cpointer C_2) {
	...
	for (std::list<cpair>::iterator it1 = m_pah->m_R5loc.begin(); it1 != m_pah->m_R5loc.end(); ++it1) {
		...
		if (dist1 <= bond_length && dist2 <= bond_length && dist3 <= bond_length*1.5) return true;
	}
	return false;
...
void PAHProcess::updateCombinedSitesMigration(Spointer& st) {
	...
	case 500:{
		...
		//if (remR5coords) m_pah->m_R5loc.push_back(R5coords);
...
void PAHProcess::proc_G6R_AC(Spointer& stt, Cpointer C_1, Cpointer C_2) {
    ...
    if(!(C_1->C2->bridge) || !(C_2->C1->bridge)) { // check if bulk C in AC site is a bridge
		...
		cpair lat_direction = add_vector(AJvector, invert_vector(AGvector));
		double dist = getDistance_twoC(C_1,C_2);
		Spointer other_side = findSite(C_2->C1->C1);
		...
		if (other_side!=m_pah->m_siteList.end()){
			Spointer S1_os = moveIt(other_side, -1);
			Spointer S2_os = moveIt(other_side, +1);
			Spointer S3_os = moveIt(other_side, -2);
			Spointer S4_os = moveIt(other_side, +2);
			updateCombinedSites(other_side); updateCombinedSites(S1_os); updateCombinedSites(S2_os);
			updateCombinedSites(S3_os); updateCombinedSites(S4_os);
		}
    }
    //printStruct();
    // neighbouring sites:
...
void PAHProcess::proc_C5R_RAC(Spointer& stt, Cpointer C_1, Cpointer C_2) {
	...
	if (angle_norm_vectors < 0.30 || getDistance_twoC(C2_new,C2_new->C2) < 1.1 || getDistance_twoC(C2_new,C2_new->C2) > 1.80 || bridge){
		OpenBabel::OBMol mol = passPAH();
		mol = optimisePAH(mol);
		passbackPAH(mol);
...
void PAHProcess::proc_L5R_BY5(Spointer& stt, Cpointer C_1, Cpointer C_2) {
	...
	//Optimise if needed
	if (getDistance_twoC(C_1, C_2) > 2.6 || bridged_before){
		if (!m_pah->m_optimised){
			OpenBabel::OBMol mol = passPAH();
			mol = optimisePAH(mol);
			passbackPAH(mol);
...
void PAHProcess::proc_M5R_ACR5_ZZ_light(Spointer& stt, Cpointer C_1, Cpointer C_2) {
	...
	if (b4) {
		...
		if (steps == 0){
			CR5_otherside_1 = C_1->C2->C2;
			CR5_otherside_2 = C_1->C2;
			if (CRem->C1->A=='C') CRem_next = CRem->C1;
			else CRem_next = CRem->C1->C1;
		} else if (steps>0){
			CR5_otherside_1 = C_1->C2;
		...
		if (steps == 0){
			CR5_otherside_1 = C_2->C1->C1;
			CR5_otherside_2 = C_2->C1;
			if (CRem->C2->A=='C') CRem_next = CRem->C2;
			else CRem_next = CRem->C2->C2;
		} else if (steps<0) {
			CR5_otherside_1 = C_2->C1;
	...
	if (opp_site_bool || opp_site_bool_second || opp_site_bool_after){
		if (steps==0){
			if (b4) {
				if (CR5_otherside_2->C1->A=='H') R5coords_end = endposR5internal(CR5_otherside_2->C1->C1, CR5_otherside_2->C1);
				else R5coords_end = endposR5internal(CR5_otherside_2->C1, CR5_otherside_2,true);
			}
			else {
				if (CR5_otherside_2->C2->A=='H') R5coords_end = endposR5internal(CR5_otherside_2, CR5_otherside_2->C2);
				else R5coords_end = endposR5internal(CR5_otherside_2, CR5_otherside_2->C2,true);
			}
		}
		else if (steps<0){
			if (b4) {
	...
	if (opp_site_bool || opp_site_bool_second || opp_site_bool_after){
		...
		if (jj_opp>=0) {
			...
			if ((int)opp_site->type>=2003 && std::get<2>(m_pah->m_R5walker_sites[jj_opp])==0){
				if (isR5internal(opp_site->C1->C2,opp_site->C1->C2->C2)) {
					cpair R5_coords_rem = findR5internal(opp_site->C1->C2,opp_site->C1->C2->C2);
				} else if (isR5internal(opp_site->C2->C1->C1,opp_site->C2->C1)) {
					cpair R5_coords_rem = findR5internal(opp_site->C2->C1->C1,opp_site->C2->C1);
				}
		...
		if (jj_opp_second>=0) {
			...
			if ((int)opp_site_second->type>=2003 && std::get<2>(m_pah->m_R5walker_sites[jj_opp_second])==0){
				if (isR5internal(opp_site_second->C1->C2,opp_site_second->C1->C2->C2)) {
					cpair R5_coords_rem = findR5internal(opp_site_second->C1->C2,opp_site_second->C1->C2->C2);
				} else if (isR5internal(opp_site_second->C2->C1->C1,opp_site_second->C2->C1)) {
					cpair R5_coords_rem = findR5internal(opp_site_second->C2->C1->C1,opp_site_second->C2->C1);
				}
				m_pah->m_R5loc.push_back(R5coords_end);
			}
		}
		if (jj_opp_after>=0 && jj_opp_after!= jj_opp_second) {
			...
			/*if ((int)opp_site_after->type>=2003 && std::get<2>(m_pah->m_R5walker_sites[jj_opp_after])==0){
				...
				} else if (isR5internal(opp_site_after->C2->C1->C1,opp_site_after->C2->C1)) {
					cpair R5_coords_rem = findR5internal(opp_site_after->C2->C1->C1,opp_site_after->C2->C1);
				}
				m_pah->m_R5loc.push_back(R5coords_end);
			}*/
		}
	...
	if (opp_site_bool && !opp_site_bool_second && !opp_site_bool_after) {
		...
		if (opp_site == opp_site_second){
			...
			else if ( (int)opp_site->type >= 2003 && (int)opp_site->type <= 2005) updateSites(opp_site, opp_site->C1, opp_site->C2, 0);
			else updateSites(opp_site, opp_site->C1, opp_site->C2, -2000);
			Spointer S1_opp_site = moveIt(opp_site, -1);
			Spointer S2_opp_site = moveIt(opp_site, +1);
			updateCombinedSitesMigration(opp_site); updateCombinedSitesMigration(S1_opp_site);  updateCombinedSitesMigration(S2_opp_site); 
...
void PAHProcess::proc_M5R_ACR5_ZZ_ZZ_light(Spointer& stt, Cpointer C_1, Cpointer C_2, rng_type &rng) {
	...
	if (opp_site_bool && !opp_site_bool_second && !opp_site_bool_after){
		...
		int jj = findWalker(opp_site_second);
		if ((int)opp_site_second->type>=2003 && std::get<2>(m_pah->m_R5walker_sites[jj])==0){
			if (isR5internal(opp_site_second->C1->C2,opp_site_second->C1->C2->C2)) {
				cpair R5_coords_rem = findR5internal(opp_site_second->C1->C2,opp_site_second->C1->C2->C2);
			} else if (isR5internal(opp_site_second->C2->C1->C1,opp_site_second->C2->C1)) {
				cpair R5_coords_rem = findR5internal(opp_site_second->C2->C1->C1,opp_site_second->C2->C1);
...
void PAHProcess::moveWalker(int ii){
	...
	if (sFE2->type==FE || sFE2->type==R5R6){
		...
		if (b4) {
			std::get<0>(m_pah->m_R5walker_sites[ii]) = S1;
			std::get<1>(m_pah->m_R5walker_sites[ii]) = sFE2;
			std::get<2>(m_pah->m_R5walker_sites[ii]) = 0;
		} else{
			std::get<0>(m_pah->m_R5walker_sites[ii]) = sFE2;
			std::get<1>(m_pah->m_R5walker_sites[ii]) = S2;
			std::get<2>(m_pah->m_R5walker_sites[ii]) = 0;
		}
	} else{
...
bool PAHProcess::checkSiteMigration(Spointer stt, bool b4){
	...
	if (CR5_otherside->bridge) return false;
	if (CR5_otherside->C2->bridge) return false; 
	if (CR5_otherside->C1->bridge) return false;
	if (CR5_otherside->C2->C2->bridge) return false; 
	if (CR5_otherside->C1->C1->bridge) return false;
...
int PAHProcess::findWalker(Spointer current_site){
	if (m_pah->m_R5walker_sites.size()==0) return -9999; //-9999 is returned when there are no walkers declared.
	int ii = -1000;
	...
	return ii; //-1000 is returned when there are declared walkers but the site called is no walker.
