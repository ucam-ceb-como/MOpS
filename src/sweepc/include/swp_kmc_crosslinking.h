 ...
 * Sourceforge:    http://sourceforge.net/projects/mopssuite
  ...
  Copyright (C) 2008 Gustavo Leon

  File purpose:
    Definition of the crosslinking coagulation process.  This coagulation process uses a crosslinking
    kernel (REF). 
...
class CrosslinkingCoagulation : public Coagulation
{
public:
    // Constructors.
    CrosslinkingCoagulation(const Sweep::Mechanism &mech); // Default constructor.

    CrosslinkingCoagulation(                     // Stream-reading constructor.
        std::istream &in,            //  - Input stream.
        const Sweep::Mechanism &mech //  - Parent mechanism.
        );
    
    //* Virtual destructor
    virtual ~CrosslinkingCoagulation() {};
    
    virtual CrosslinkingCoagulation* const Clone() const;

    //* Returns the process type for identification during serialisation
    virtual ProcessType ID(void) const {return Crosslinking_Coagulation_ID;};
    ...
    //! Perform a coagulation with particles chosen according to the crosslinking kernel
    virtual int Perform(
        double t,
        Cell &sys,
        const Geometry::LocalGeometry1d& local_geom,
...
protected:
    //! Crosslinking coagulation kernel between two particles
