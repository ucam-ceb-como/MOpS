...
// Default constructor.
EnsembleImgNode::EnsembleImgNode(void)
...
// children.
void EnsembleImgNode::Clear(void)
{
    // Set size to zero.
    m_r  = 0.0;
    m_r2 = 0.0;
...
void EnsembleImgNode::CopySPT(const SubParticle *sp)
{
	Primary::PropID id=Primary::iD;

    if (sp->Primary()== NULL) {

            m_left = new EnsembleImgNode();
            m_left->m_parent = this;
			m_left->CopySPT(sp->Left());

            m_right = new EnsembleImgNode();
...
void EnsembleImgNode::Insert(real radius,Particle *sp)
{
    ...
    if ((m_left == NULL) && (m_right == NULL)) {
        ...
        if (m_left->m_mindepth <= m_right->m_mindepth) {
            ...
            m_left->Insert(radius,sp);
        } else {
            // Add primary to the right branch if it is
            // shallower.
            m_right->Insert(radius,sp);
        }
...
bool EnsembleImgNode::IsLeaf(void) const
{
...
void EnsembleImgNode::GetPriCoords(std::vector<fvector> &coords) const
{
    if (IsLeaf()) {
        fvector c(4);
        c[0] = m_cen_mass[0];
...
void EnsembleImgNode::Translate(real dx, real dy, real dz)
{
...
void EnsembleImgNode::RotateCOM(real dtheta, real dphi)
{
...
void EnsembleImgNode::RotateBoundSph(real dtheta, real dphi)
{
...
void EnsembleImgNode::RotateOrigin(real dtheta, real dphi)
{
    // Create transformation matrix.
    Coords::Matrix M;
    M.SetIdentity();
...
void EnsembleImgNode::transform(const Coords::Matrix &mat)
{
    // Rotate child nodes.
...
const Coords::Vector &EnsembleImgNode::BoundSphCentre(void) const
{
    return m_cen_bsph;
}

// Returns the bounding sphere radius.
real EnsembleImgNode::Radius(void) const
{
    return m_r;
}

// Sets the radius of the bounding sphere.
void EnsembleImgNode::setRadius(real r)
{
    m_r  = r;
    m_r2 = r * r;
    m_r3 = m_r2 * m_r;
...
void EnsembleImgNode::CalcBoundSph(void)
{
    if ((m_left != NULL) && (m_right != NULL)) {
...
const Coords::Vector &EnsembleImgNode::CentreOfMass(void) const
{
    return m_cen_mass;
}
...
// node values.
void EnsembleImgNode::CalcCOM(void)
{
    if ((m_left != NULL) && (m_right != NULL)) {
