...
ActSiteReaction::ActSiteReaction(std::istream &in)
{
    Deserialize(in);
}
...
// Assignment operator.
ActSiteReaction &ActSiteReaction::operator =(const Sweep::ActSiteReaction &rhs)
{
    if (this != &rhs) {
...
void ActSiteReaction::Deserialize(std::istream &in)
{
    if (in.good()) {
        ...
        switch (version) {
            case 0:
                ...
                SurfaceReaction::Deserialize(in);

                // Read the active sites model ID.
                in.read(reinterpret_cast<char*>(&n), sizeof(n));
                if (n==1) {
                    in.read(reinterpret_cast<char*>(&id), sizeof(id));
                    m_asmodel = ModelFactory::GetActSitesModel((ModelType)id);
                } else {
                    m_asmodel = NULL;
                }
