 ...
 */
 Sweep::real Sweep::Processes::SecondaryPrimaryCoag::RateTerms(real t, const Cell &sys,
     ...
     *iterm = r * numSecondaryParticles * invRootMinMass * sys.Particles().GetSum(ParticleCache::iD2);
     // Add it to the total and move the iterator on ready to receive the next rate term value
     sum += *iterm++;

     // Calculate the third rate term
     *iterm = r * numSecondaryParticles * maxCollDiam2 * sys.Particles().GetSum(ParticleCache::iM_1_2);
     // Add it to the total and move the iterator on ready to receive the next rate term value
     sum += *iterm++;

     // Calculate the fourth rate term
     *iterm = r * numSecondaryParticles *  sys.Particles().GetSum(ParticleCache::iD2_M_1_2);
     // Add it to the total and move the iterator on ready to receive the next rate term value
     sum += *iterm++;
                              ...
                              Sweep::Transport::TransportOutflow *out) const
{
    ...
    switch (iterm) {
        ...
        case 1:
            ...
            index1 = sys.Particles().Select(ParticleCache::iD2, rand_int , rand_u01);
            break;
        case 2:
            // Select by 1 over square root of mass
            index1 = sys.Particles().Select(ParticleCache::iM_1_2, rand_int , rand_u01);
            break;
        case 3:
            // Select by 1 over square root of mass times collision diameter squared
            index1 = sys.Particles().Select(ParticleCache::iD2_M_1_2, rand_int , rand_u01);
            break;
        default:
