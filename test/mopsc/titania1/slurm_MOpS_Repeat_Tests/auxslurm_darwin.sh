...
do
	dirname=swa$t
        mkdir $dirname
        cd $dirname
	srun --exclusive --ntasks=1 $2 --diags -p -w --strang --ensemble -t ../therm.dat -r ../mops-hm-s1.xml -c ../chem.inp -s ../sweep-fo-detailed-w3-$t.xml &
        cd ..
done
for t in '1' '2' '3' '4' '5'
do
	dirname=dsa$t
        mkdir $dirname
        cd $dirname
	srun --exclusive --ntasks=1 $2 --diags -p -w --strang --ensemble -t ../therm.dat -r ../mops-hm-s1.xml -c ../chem.inp -s ../sweep-fo-detailed-$t.xml &
        cd ..
done
wait
cd ../../
