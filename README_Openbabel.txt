...
### Install WSL (Ubuntu)
The main procedure can be found on https://docs.microsoft.com/en-us/windows/wsl/install
On an elevated Powershell:
    wsl --install
    Setup an user account and password
    Allow for the distribution to download and run

Following commands from Linux terminal

### General Ubuntu updates
    sudo apt update && sudo apt upgrade

### Essential commands, compiler, g++, etc.
...
Alternatively it can be downloaded with:
...
The repositories for GCC4 are not easy to obtain. The following method can become obsolete at some point.
...
Possibly remove xenial repositories from /etc/apt/sources.list
Tell the Linux distribution to prioritise gcc4. This will probably make gcc4 the preferred compiler for every executable so the update-alternatives parameters can be changed later.
...
From this point all the necessary libraries and packages should be installed. Follow the Installation procedure detailed on INSTALLATION.md
...
Add the C++ extension for VS Code https://code.visualstudio.com/docs/languages/cpp
Start debugging C++ code. For example see https://code.visualstudio.com/docs/cpp/config-wsl
